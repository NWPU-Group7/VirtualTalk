...
public class FaceDetector {
    ...
    public void solveFace(Bitmap bmp) {
        ...
        synchronized (this) {
            ...
            if (landmarks == null || landmarks.size() == 0)
                Log.d(TAG, "no landmarks");

            canvas.drawRect(ret.getLeft(), ret.getTop(), ret.getRight(), ret.getBottom(), mLandmarkPaint);
...
//        if(yaw < Math.PI / 18) {
    ...
    private void solveEmotion(ArrayList<Point> landmarks, int width) {
        //mouth open
        emotion[3] = (double)(landmarks.get(57).y-landmarks.get(51).y) /
